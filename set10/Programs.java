...
public class Programs {
    ...
    public static void addDefinitionsToEnvironment(Map<String, ExpVal> env, 
    	 ...
    	 for (int index=0; index < defs.size(); index++) {
             ...
             if (definition.rhs().isLambda()) {
                 FunVal funVal = Asts.expVal(definition.rhs().asLambda(), 
                		 					 env);
                 env.put(definition.lhs(), funVal);
             } 
             else if(definition.rhs().isConstant()){
     			env.put(definition.lhs(), 
     					definition.rhs().asConstant().value());
     		}
     		else{		
     			env.put(definition.lhs(), 
     					Asts.expVal(Asts.lambdaExp(null, 
     											   definition.rhs()), 
     											   env));
     		}
         }
    ...
    public static void main(String[] args) {
    	PrrogramsTests.testArithmeticOperations();
    	PrrogramsTests.testIfOperations();
    	PrrogramsTests.testProgram();
    	PrrogramsTests.testDef();
    }
