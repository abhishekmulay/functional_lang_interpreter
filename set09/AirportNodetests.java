class AirportNodeTests {
  public static void main(String[] args) {
    ...
    //test setPriority
    assert (new AirportNode("BOS", 2)).setPriority(18).getPriority()
        == 18;
    assert (new AirportNode("LAX", 20)).setPriority(4).getPriority()
        == 4;

    //test setName
    assert (new AirportNode("BOS", 2)).setName("DET").getName()
        .equals("DET");
    assert (new AirportNode("LAX", 20)).setName("LGA").getName()
        .equals("LGA");

    // test equals
    assert (new AirportNode("BOS", 2))
        .equals(new AirportNode("BOS", 2));

    assert (new AirportNode("LGA", 20))
        .equals(new AirportNode("BOS", 2)) == false;
